...
// ── Avatar helper ──────────────────────────────────────────────
function Avatar({ uri, name, isAnon, size = 40, style }) {
  const initials = isAnon ? '?' : (name || '?').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  const bg = isAnon ? colors.ink3 : colors.green;
  if (uri) {
      ...
      <Pressable style={ps.overlay} onPress={onClose}>
        <Pressable style={ps.sheet} onPress={() => {}}>
          ...
          <View style={ps.header}>
            <Avatar
              uri={isAnon ? null : (profile?.avatar_url || person.avatar_url)}
              name={name}
              isAnon={isAnon}
              size={64}
...
const ps = StyleSheet.create({
  ...
  btnSoft:       { backgroundColor: '#fff', borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16, alignItems: 'center', borderWidth: 1, borderColor: colors.line },
...
function CreateChatSheet({ nearbyUsers, myUserId, onClose, onCreated }) {
  ...
  const toggle = (userId) => setSelected(s => {
    const n = new Set(s);
    ...
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={cc.overlay} onPress={onClose}>
        <Pressable style={cc.sheet} onPress={() => {}}>
          ...
          {selected.size > 1 && (
            <TextInput style={cc.nameInput} value={name} onChangeText={setName}
              placeholder="Group name (optional)" placeholderTextColor={colors.ink3} />
          )}
          ...
          <TouchableOpacity
            ...
            onPress={create} disabled={selected.size === 0 || creating}
          >
            {creating
...
const cc = StyleSheet.create({
  ...
  sheet:            { backgroundColor: colors.cream, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 36, maxHeight: '80%' },
  grab:             { width: 36, height: 4, backgroundColor: colors.line, borderRadius: 99, alignSelf: 'center', marginBottom: 16 },
  ...
  nameInput:        { backgroundColor: '#fff', borderWidth: 1.5, borderColor: colors.green, borderRadius: 14, padding: 12, fontSize: 15, color: colors.ink, marginBottom: 12 },
  list:             { maxHeight: 300, marginBottom: 16 },
  person:           { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, backgroundColor: '#fff', borderRadius: 14, marginBottom: 8, borderWidth: 1.5, borderColor: colors.line },
  ...
  check:            { width: 24, height: 24, borderRadius: 12, borderWidth: 1.5, borderColor: colors.line, alignItems: 'center', justifyContent: 'center' },
...
function NearbySheet({ people, onTap, onTagFilter, activeTag, onCreateChat }) {
  ...
  const snapTo = (expand) => {
    ...
    Animated.spring(animHeight, { toValue: expand ? EXPANDED : COLLAPSED, useNativeDriver: false, tension: 60, friction: 10 }).start();
  };

  const panResponder = useRef(PanResponder.create({
    onStartShouldSetPanResponder: () => true,
    onMoveShouldSetPanResponder:  (_, g) => Math.abs(g.dy) > 4,
    onPanResponderGrant: () => { animHeight.stopAnimation(v => { lastHeight.current = v; }); },
    onPanResponderMove:  (_, g) => { animHeight.setValue(Math.min(Math.max(lastHeight.current - g.dy, COLLAPSED), EXPANDED)); },
  ...
  return (
    <Animated.View style={[ns.sheet, { height: animHeight, bottom: TAB_BAR_H }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={ns.cardRow}>
        {people.length === 0
...
const ns = StyleSheet.create({
  sheet:            { position: 'absolute', left: 0, right: 0, zIndex: 20, backgroundColor: '#fff',
  ...
  grab:             { width: 36, height: 4, backgroundColor: colors.line, borderRadius: 99, alignSelf: 'center', marginBottom: 10 },
  ...
  tagChip:          { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 99, backgroundColor: colors.cream, borderWidth: 1, borderColor: colors.line, alignSelf: 'flex-start' },
    ...
    <View style={s.container}>
            ...
            >
              ...
              <View style={[s.pin, { backgroundColor: isAnon ? colors.ink3 : colors.green }]}>
                {!isAnon && person.avatar_url ? (
                  ...
                  <Text style={s.pinText}>{name}</Text>
                )}
              </View>
            </Marker>
          );
      ...
      {showCreateChat && (
        <CreateChatSheet
          ...
          onCreated={(roomId) => { setShowCreateChat(false); router.push(`/chat/${roomId}`); }}
        />
      )}
    </View>
  );
...
const s = StyleSheet.create({
  ...
  topBar:             { position: 'absolute', top: 56, left: 12, right: 12, zIndex: 10, flexDirection: 'row', gap: 8, alignItems: 'center' },
  topPill:            { flex: 1, height: 44, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 22, borderWidth: 1, borderColor: 'rgba(20,32,25,0.06)', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, gap: 6, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  ...
  ghostBtn:           { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.92)', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  ghostBtnText:       { fontSize: 20 },
  zoomBtns:           { position: 'absolute', right: 16, top: '42%', zIndex: 12, backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 14, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 8, elevation: 4 },
  ...
  radiusBar:          { position: 'absolute', bottom: 100 + 12, left: 16, right: 16, zIndex: 12, backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 16, padding: 12, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 8, elevation: 4 },
  radiusLabel:        { fontSize: 11, fontWeight: '700', color: colors.ink3, fontFamily: 'monospace', letterSpacing: 1 },
  radiusValue:        { fontSize: 13, color: colors.ink, fontFamily: 'monospace', marginBottom: 8 },
  radiusBtns:         { flexDirection: 'row', gap: 6 },
  radiusBtn:          { flex: 1, paddingVertical: 7, borderRadius: 99, backgroundColor: colors.cream, borderWidth: 1, borderColor: colors.line, alignItems: 'center' },
